...
The frontend is configured using:

VITE_API_BASE_URL=http://localhost:3000


Therefore, the local API is expected to be available at:
...
Return a useful message on errors.

Validate IDs and request payloads.

Validate uploaded portfolio files.

Validate geographic boundaries server-side.

Do not rely exclusively on frontend validation.

Keep geographic calculations consistent between API and UI.

Make discovery operations safe to retry where possible.

Keep response structures backwards-compatible when practical.
