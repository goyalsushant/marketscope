...
The current UI enforces a maximum market boundary area of:
...
UI sends the file to the API.

API processes the file.

API returns an upload ID.

UI stores the upload ID.

The upload ID is submitted when creating the market.
...
When VITE_API_BASE_URL is not provided, the market setup API currently falls back to:
...
Start the development server using the command defined by the API project:

npm run dev


The API should be accessible from the URL configured in the UI.
